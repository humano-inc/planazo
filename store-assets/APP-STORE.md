...
What to try
  ...
  2. Open "Rooftop pizza night" and tap "I'm in". It needs four people and
     has three, so your answer is the one that confirms it: the slot bar
     fills and the status changes.
  3. Open "Padel, some weeknight", tick a few evenings, then "Send my dates".
     This is a plan with no date yet, decided by whichever evening suits the
     most people.
...
**The account exists on production and is seeded.** `pnpm db:seed:review`
rebuilds it, and it is safe to re-run: accounts and groups are matched on email
and invite code, and only the plans inside the two review groups are replaced.

```bash
REVIEW_SEED_CONFIRM=leszgvpjonzjclhbgzju \
  REVIEW_SEED_PASSWORD='<the password in the password manager>' \
  SUPABASE_URL=https://leszgvpjonzjclhbgzju.supabase.co \
  SUPABASE_SERVICE_ROLE_KEY=$(supabase projects api-keys \
    --project-ref leszgvpjonzjclhbgzju -o json \
    | jq -r '.[] | select(.name=="service_role") | .api_key') \
  pnpm db:seed:review
```

The password lives in the password manager, never in this file, and goes into
App Store Connect and Play's App access field. The seed sets it on every run,
so changing it there changes it everywhere.

It is not `pnpm db:seed:demo`. That one is built for a throwaway database: it
grants its primary account app-admin, writes fake feedback, and when
`SEED_PRIMARY_EMAIL` is unset it adopts the first existing auth user, which on
production is a real person. Reviewers reject on an empty app, but not as
badly as your users would reject finding themselves in four invented groups.

What the reviewer gets: two groups (Weekend Crew, where they are an admin, and
Food & Drinks) with five other members, one plan waiting on their answer, one
plan with no date yet, one already past its minimum, and one night that has
already happened so the album has somewhere to live.
...
| Play Console account, as the **organisation** | done | registered 2026-08-12; an org account is exempt from the closed-testing gate that personal accounts must clear |
| `google-services.json` | done | Firebase project `planazo-487f1`, committed at `apps/mobile/google-services.json` |
| FCM V1 service account key | done | uploaded to Expo, see §7.2 |
| Upload keystore | done | generated by EAS, Build Credentials `GwmvQ8RFxX` for `com.somosplanazo.app`; the older `8WXCPNZ8au` belongs to the abandoned `com.planazo.app`. Upload certificate SHA-256 `E8:7F:68:92:44:CD:D7:8D:ED:A6:79:38:9F:58:0D:DC:98:65:E7:18:19:22:99:32:64:BF:5E:41:35:6B:BD:53`. Play binds the app to whatever key signs the first AAB, so this one can never be regenerated; `keytool -printcert -jarfile <the .aab>` shows which key signed a build |
| Demo account for review | done | the same seeded `review@planazo.me` as §5 |
...
- **`RECORD_AUDIO`** is added by `expo-image-picker`'s config plugin unless you
  say otherwise, on the assumption that a picker records video. Planazo picks
  `mediaTypes: ['images']` everywhere and `takePhoto()` takes a still, so
  `microphonePermission: false` in the plugin's options both skips it and
  blocks anything else from adding it. On iOS the same option drops
  `NSMicrophoneUsageDescription`.
- **`SYSTEM_ALERT_WINDOW`** ("Display over other apps") comes from Expo's bare
  Android template, not from any dependency, so only
